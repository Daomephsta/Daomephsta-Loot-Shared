...
public class CommandLootTables extends CraftTweakerCommand
{
    ...
    protected void init()
    {
        setDescription(DaomephstaLootShared.translation(".commands.dump.desc"));
    }
    ...
    public void executeCommand(MinecraftServer server, ICommandSender sender, String[] args)
    {
        if (args.length < 1 || (args.length == 1 && args[0].equals("help")))
        {
            for (String subcommand : subcommands.keySet())
                sender.sendMessage(DaomephstaLootShared.translation(".commands.dump." + subcommand  + ".usage", style -> style.setColor(TextFormatting.RED)));
            sender.sendMessage(DaomephstaLootShared.translation(".commands.dump.openDocs", Texts.Styles.urlLink(DOCS_URL)));
            return;
        }
        ...
        if (subcommand != null)
        {
            ...
            sender.sendMessage(DaomephstaLootShared.translation(".commands.dump.unknownSubcommand", args[0]));
    }
